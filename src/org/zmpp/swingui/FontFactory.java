/*
 * $Id: FontFactory.java 535 2008-02-19 06:02:50Z weiju $
 * 
...
public class FontFactory {
    ...
    private ScreenFont getStyle(int fontnum, int oldstyle, int style) {
        ...
        if (style == ScreenModel.TEXTSTYLE_ROMAN) {
            return fonts[fontnum - 1][0];
        } else {
    ...
    private void deriveFont(int fontnum, int style) {
        ...
        if ((style & ScreenModel.TEXTSTYLE_BOLD) > 0) {
            fontstyle |= Font.BOLD;
        }
        if ((style & ScreenModel.TEXTSTYLE_ITALIC) > 0) {
            fontstyle |= Font.ITALIC;
        }
        if ((style & ScreenModel.TEXTSTYLE_REVERSE_VIDEO) > 0) {
            reverse = true;
        }
