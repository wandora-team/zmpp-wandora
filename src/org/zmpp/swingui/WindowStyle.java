/*
 * $Id: WindowStyle.java 535 2008-02-19 06:02:50Z weiju $
 * 
...
public class WindowStyle {
    ...
    public void setIsWrapped(boolean flag) {
        isWrapped = flag;
    }
    ...
    public int getFlags() {
        ...
        int result = 0;
        if (isWrapped) {
            result |= FLAG_WORD_WRAP;
        }
        if (isScrolled) {
            result |= FLAG_SCROLL;
        }
        if (isCopiedToStream2) {
            result |= FLAG_COPYTO_STREAM2;
        }
        if (isBuffered) {
            result |= FLAG_BUFFEROUTPUT;
        }
    ...
    private void setBits(int flags) {
        ...
        if ((flags & FLAG_WORD_WRAP) > 0) {
            isWrapped = true;
        }
        if ((flags & FLAG_SCROLL) > 0) {
            isScrolled = true;
        }
        if ((flags & FLAG_COPYTO_STREAM2) > 0) {
            isCopiedToStream2 = true;
        }
        if ((flags & FLAG_BUFFEROUTPUT) > 0) {
            isBuffered = true;
        }
    }
    ...
    private void reverseBits(int flags) {
        ...
        if ((flags & FLAG_COPYTO_STREAM2) > 0) {
            isCopiedToStream2 = !isCopiedToStream2;
        }
        if ((flags & FLAG_BUFFEROUTPUT) > 0) {
